...
angular.module('Drippio.services', ['ngResource', 'Drippio.config']).
  ...
  factory( 'SessionHandler', ['$cookies', '$location', function( $cookies, $location ) {
    var sessionHandler = {};
    sessionHandler.setToken = function( newToken ) {
      $cookies.authToken = newToken;
    };
    sessionHandler.getToken = function() {
      if( !angular.isDefined( $cookies.authToken ) ) {
        return '';
      } else {
    ...
    sessionHandler.isTokenSet = function() {
      return angular.isDefined( $cookies.authToken );
    }
    //
        ...
        getCurrentUser: function( callback ) {
          if( !angular.isDefined( auth.currentUser ) ) {
            User.load( {}, function success( user ) {
              ...
              SessionHandler.setToken( user.auth_token );
              $rootScope.currentUser = auth.currentUser;
              if( callback !== undefined ) callback( auth );
              if( $location.path() == '/login' ) $location.path( '/drip_marketing_campaigns' );
            }, function error() {
